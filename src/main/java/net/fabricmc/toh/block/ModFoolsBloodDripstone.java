...
public class ModFoolsBloodDripstone extends PointedDripstoneBlock {
    ...
    public ModFoolsBloodDripstone(AbstractBlock.Settings settings) {
        super(settings);
    ...
    public boolean canPlaceAt(BlockState state, WorldView world, BlockPos pos) {
        return ModFoolsBloodDripstone.canPlaceAtWithDirection(world, pos, state.get(VERTICAL_DIRECTION));
    }
    ...
    public BlockState getStateForNeighborUpdate(BlockState state, Direction direction, BlockState neighborState, WorldAccess world, BlockPos pos, BlockPos neighborPos) {
        ...
        Thickness thickness = ModFoolsBloodDripstone.getThickness(world, pos, direction2, bl);
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        ModFoolsBloodDripstone.getFluid(world, pos, state).filter(fluid -> f < 0.02f || ModFoolsBloodDripstone.isFluidLiquid(fluid.fluid)).ifPresent(fluid -> ModFoolsBloodDripstone.createParticle(world, pos, state, fluid.fluid));
    }

    @Override
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (ModFoolsBloodDripstone.isPointingUp(state) && !this.canPlaceAt(state, world, pos)) {
            world.breakBlock(pos, true);
        } else {
            ModFoolsBloodDripstone.spawnFallingBlock(state, world, pos);
        }
    }

    @Override
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ModFoolsBloodDripstone.dripTick(state, world, pos, random.nextFloat());
        if (random.nextFloat() < 0.011377778f && ModFoolsBloodDripstone.isHeldByPointedDripstone(state, world, pos)) {
            ModFoolsBloodDripstone.tryGrow(state, world, pos, random);
        }
    }
    ...
    public static void dripTick(BlockState state, ServerWorld world, BlockPos pos, float dripChance) {
        ...
        if (!ModFoolsBloodDripstone.isHeldByPointedDripstone(state, world, pos)) {
            return;
        }
        Optional<ModFoolsBloodDripstone.DrippingFluid> optional = ModFoolsBloodDripstone.getFluid(world, pos, state);
        if (optional.isEmpty()) {
            return;
        }
        ...
        BlockPos blockPos = ModFoolsBloodDripstone.getTipPos(state, world, pos, 11, false);
        if (blockPos == null) {
            return;
        }
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        ...
        Direction direction2 = ModFoolsBloodDripstone.getDirectionToPlaceAt(worldAccess, blockPos = ctx.getBlockPos(), direction = ctx.getVerticalPlayerLookDirection().getOpposite());
        if (direction2 == null) {
            return null;
        }
        boolean bl = !ctx.shouldCancelInteraction();
        Thickness thickness = ModFoolsBloodDripstone.getThickness(worldAccess, blockPos, direction2, bl);
        if (thickness == null) {
            return null;
        }
    ...
    private static void spawnFallingBlock(BlockState state, ServerWorld world, BlockPos pos) {
        ...
        while (ModFoolsBloodDripstone.isPointingDown(blockState)) {
            FallingBlockEntity fallingBlockEntity = FallingBlockEntity.spawnFromBlock(world, mutable, blockState);
            if (ModFoolsBloodDripstone.isTip(blockState, true)) {
    ...
    public static void tryGrow(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (!ModFoolsBloodDripstone.canGrow(blockState, blockState2 = world.getBlockState(pos.up(2)))) {
            return;
        }
        BlockPos blockPos = ModFoolsBloodDripstone.getTipPos(state, world, pos, 7, false);
        if (blockPos == null) {
            return;
        }
        BlockState blockState3 = world.getBlockState(blockPos);
        if (!ModFoolsBloodDripstone.canDrip(blockState3) || !ModFoolsBloodDripstone.canGrow(blockState3, world, blockPos)) {
            return;
        }
        if (random.nextBoolean()) {
            ModFoolsBloodDripstone.tryGrow(world, blockPos, Direction.DOWN);
        } else {
            ModFoolsBloodDripstone.tryGrowStalagmite(world, blockPos);
        }
    }
    ...
    private static void tryGrowStalagmite(ServerWorld world, BlockPos pos) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (ModFoolsBloodDripstone.isTip(blockState, Direction.UP) && ModFoolsBloodDripstone.canGrow(blockState, world, mutable)) {
                ModFoolsBloodDripstone.tryGrow(world, mutable, Direction.UP);
                return;
            }
            if (ModFoolsBloodDripstone.canPlaceAtWithDirection(world, mutable, Direction.UP) && !world.isWater((BlockPos)mutable.down())) {
                ModFoolsBloodDripstone.tryGrow(world, (BlockPos)mutable.down(), Direction.UP);
                return;
            }
            if (ModFoolsBloodDripstone.canDripThrough(world, mutable, blockState)) continue;
            return;
        }
    }
    ...
    private static void tryGrow(ServerWorld world, BlockPos pos, Direction direction) {
        ...
        if (ModFoolsBloodDripstone.isTip(blockState, direction.getOpposite())) {
            ModFoolsBloodDripstone.growMerged(blockState, world, blockPos);
        } else if (blockState.isAir() || blockState.isOf(Blocks.WATER)) {
            ModFoolsBloodDripstone.place(world, blockPos, direction, Thickness.TIP);
        }
    }

    private static void place(WorldAccess world, BlockPos pos, Direction direction, Thickness thickness) {
        BlockState blockState = (BlockState)((BlockState)((BlockState)ModBlocks.FOOLS_BLOOD_DRIPSTONE.getDefaultState().with(VERTICAL_DIRECTION, direction)).with(THICKNESS, thickness)).with(WATERLOGGED, world.getFluidState(pos).getFluid() == Fluids.WATER);
    ...
    private static void growMerged(BlockState state, WorldAccess world, BlockPos pos) {
        ...
        ModFoolsBloodDripstone.place(world, blockPos2, Direction.DOWN, Thickness.TIP_MERGE);
        ModFoolsBloodDripstone.place(world, blockPos, Direction.UP, Thickness.TIP_MERGE);
    }

    public static void createParticle(World world, BlockPos pos, BlockState state) {
        ModFoolsBloodDripstone.getFluid(world, pos, state).ifPresent(fluid -> ModFoolsBloodDripstone.createParticle(world, pos, state, fluid.fluid));
    }
    ...
    private static void createParticle(World world, BlockPos pos, BlockState state, Fluid fluid) {
        ...
        Fluid fluid2 = ModFoolsBloodDripstone.getDripFluid(world, fluid);
    ...
    private static BlockPos getTipPos(BlockState state2, WorldAccess world, BlockPos pos2, int range, boolean allowMerged) {
        if (ModFoolsBloodDripstone.isTip(state2, allowMerged)) {
            return pos2;
        }
        Direction direction = state2.get(VERTICAL_DIRECTION);
        BiPredicate<BlockPos, BlockState> biPredicate = (pos, state) -> state.isOf(ModBlocks.FOOLS_BLOOD_DRIPSTONE) && state.get(VERTICAL_DIRECTION) == direction;
        return ModFoolsBloodDripstone.searchInDirection(world, pos2, direction.getDirection(), biPredicate, state -> ModFoolsBloodDripstone.isTip(state, allowMerged), range).orElse(null);
    }

    @Nullable
    private static Direction getDirectionToPlaceAt(WorldView world, BlockPos pos, Direction direction) {
        Direction direction2;
        if (ModFoolsBloodDripstone.canPlaceAtWithDirection(world, pos, direction)) {
            direction2 = direction;
        } else if (ModFoolsBloodDripstone.canPlaceAtWithDirection(world, pos, direction.getOpposite())) {
    ...
    private static Thickness getThickness(WorldView world, BlockPos pos, Direction direction, boolean tryMerge) {
        ...
        if (ModFoolsBloodDripstone.isPointedDripstoneFacingDirection(blockState, direction2)) {
            if (tryMerge || blockState.get(THICKNESS) == Thickness.TIP_MERGE) {
                return Thickness.TIP_MERGE;
            }
            return Thickness.TIP;
        }
        if (!ModFoolsBloodDripstone.isPointedDripstoneFacingDirection(blockState, direction)) {
            return Thickness.TIP;
        }
        ...
        if (!ModFoolsBloodDripstone.isPointedDripstoneFacingDirection(blockState2, direction)) {
    ...
    public static boolean canDrip(BlockState state) {
        return ModFoolsBloodDripstone.isPointingDown(state) && state.get(THICKNESS) == Thickness.TIP && state.get(WATERLOGGED) == false;
    }
    ...
    private static boolean canGrow(BlockState state, ServerWorld world, BlockPos pos) {
        ...
        return ModFoolsBloodDripstone.isTip(blockState, direction.getOpposite());
    }

    private static Optional<BlockPos> getSupportingPos(World world, BlockPos pos2, BlockState state2, int range) {
        Direction direction = state2.get(VERTICAL_DIRECTION);
        BiPredicate<BlockPos, BlockState> biPredicate = (pos, state) -> state.isOf(ModBlocks.FOOLS_BLOOD_DRIPSTONE) && state.get(VERTICAL_DIRECTION) == direction;
        return ModFoolsBloodDripstone.searchInDirection(world, pos2, direction.getOpposite().getDirection(), biPredicate, state -> !state.isOf(ModBlocks.FOOLS_BLOOD_DRIPSTONE), range);
    }

    private static boolean canPlaceAtWithDirection(WorldView world, BlockPos pos, Direction direction) {
        BlockPos blockPos = pos.offset(direction.getOpposite());
        BlockState blockState = world.getBlockState(blockPos);
        return blockState.isSideSolidFullSquare(world, blockPos, direction) || ModFoolsBloodDripstone.isPointedDripstoneFacingDirection(blockState, direction);
    }
    ...
    private static boolean isTip(BlockState state, Direction direction) {
        return ModFoolsBloodDripstone.isTip(state, false) && state.get(VERTICAL_DIRECTION) == direction;
    }

    private static boolean isPointingDown(BlockState state) {
        return ModFoolsBloodDripstone.isPointedDripstoneFacingDirection(state, Direction.DOWN);
    }

    private static boolean isPointingUp(BlockState state) {
        return ModFoolsBloodDripstone.isPointedDripstoneFacingDirection(state, Direction.UP);
    }

    private static boolean isHeldByPointedDripstone(BlockState state, WorldView world, BlockPos pos) {
        return ModFoolsBloodDripstone.isPointingDown(state) && !world.getBlockState(pos.up()).isOf(ModBlocks.FOOLS_BLOOD_DRIPSTONE);
    }
    ...
    public static BlockPos getDripPos(World world, BlockPos pos2) {
        BiPredicate<BlockPos, BlockState> biPredicate = (pos, state) -> ModFoolsBloodDripstone.canDripThrough(world, pos, state);
        return ModFoolsBloodDripstone.searchInDirection(world, pos2, Direction.UP.getDirection(), biPredicate, ModFoolsBloodDripstone::canDrip, 11).orElse(null);
    }

    public static Fluid getDripFluid(ServerWorld world, BlockPos pos) {
        return ModFoolsBloodDripstone.getFluid(world, pos, world.getBlockState(pos)).map(fluid -> fluid.fluid).filter(ModFoolsBloodDripstone::isFluidLiquid).orElse(Fluids.EMPTY);
    }

    private static Optional<ModFoolsBloodDripstone.DrippingFluid> getFluid(World world, BlockPos pos2, BlockState state) {
        if (!ModFoolsBloodDripstone.isPointingDown(state)) {
            return Optional.empty();
        }
        return ModFoolsBloodDripstone.getSupportingPos(world, pos2, state, 11).map(pos -> {
            ...
            return new ModFoolsBloodDripstone.DrippingFluid(blockPos, fluid, blockState);
        });
    }
    ...
    private static boolean canGrow(BlockState dripstoneBlockState, BlockState waterState) {
        return dripstoneBlockState.isOf(ModBlocks.FOOLS_BLOOD_BLOCK) && waterState.isOf(Blocks.WATER) && waterState.getFluidState().isStill();
    }
